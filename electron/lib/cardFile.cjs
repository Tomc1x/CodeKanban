...
function parseCardFile(raw, filename) {
  ...
  return {
    ...
    dependsOn: Array.isArray(data.depends_on) ? data.depends_on.map(String) : [],
    askUserQuestions: typeof data.ask_user_questions === 'boolean' ? data.ask_user_questions : null,
...
function serializeCard(card) {
  const frontmatter = {
    ...
    ask_user_questions: typeof card.askUserQuestions === 'boolean' ? card.askUserQuestions : null,
    refined: card.refined === true,
